...
const URGENT_NEEDS = [
  "Tarpaulins, Water",
  "Food rations, Hygiene kits",
  "Medical kits, Shelter toolkits",
...
export const ASSESSMENT_SCHEMAS: Record<
  string,
  {
    headers: string[];
...
> = {
  ...
  asm_08: {
    ...
    generateRow: (idx, yr) => {
      ...
      const relocSites = [
        "Maewo Relocation Site A",
        "Efate Off-Grid Settlement",
        "Santo East New Settlement",
      ];
      const site = relocSites[idx % relocSites.length];
      const tenure = [
        "Customary Lease",
        "Government Assigned Land",
        "Informal Agreement",
        "Purchased Freehold",
      ][idx % 4];
      const status = ["Settled", "In Progress", "Planning"][idx % 3];
      const support = [
        "Shelter Kit + Cash Grant",
        "Food Rations & Water Tank",
        "Building Toolkits",
        "None",
      ][idx % 4];
