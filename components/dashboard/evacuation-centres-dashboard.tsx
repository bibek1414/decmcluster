...
export default function EvacuationCentresDashboard() {
  ...
  const { data: stats, isLoading: statsLoading } = useEvacuationCentresStats(selectedProvince || undefined);
  const { data: locations, isLoading: locationsLoading } = useEvacuationCentreLocations(selectedProvince || undefined);
  ...
  const readinessList = useMemo(() => {
    ...
    return [
      { name: "Owner approved", value: r.is_ec_owner_approved, color: "bg-sky-400" },
      { name: "First aid trained", value: r.first_aid_trained_person, color: "bg-amber-400" },
      { name: "Govt approved", value: r.is_ec_govt_approved, color: "bg-red-500" },
      { name: "First aid kit", value: r.first_aid_kit_availability, color: "bg-red-500" },
      { name: "Kitchen", value: r.kitchen_cooking_facilities, color: "bg-red-500" },
  ...
  const washFacilityList = useMemo(() => {
    ...
    return [
      { name: "Water storage sites", value: w.water_storage_sites, color: "bg-amber-400" },
      { name: "Mens toilets", value: w.mens_toilet, color: "bg-blue-600" },
      { name: "Womens toilets", value: w.female_toilet, color: "bg-blue-600" },
      { name: "Unisex toilets", value: w.total_unisex_toilet, color: "bg-blue-600" },
      { name: "Mens showers", value: w.total_mens_shower, color: "bg-blue-600" },
      { name: "Kitchen available sites", value: w.kitchen_available_sites, color: "bg-emerald-500" },
      { name: "Womens showers", value: w.total_womens_shower, color: "bg-blue-600" },
      { name: "Laundry available sites", value: w.laundry_available_sites, color: "bg-emerald-500" },
      { name: "Disability toilets", value: w.total_disability_toilet, color: "bg-blue-600" },
      { name: "Unisex showers", value: w.total_unisex_shower, color: "bg-blue-600" }
    ];
  }, [stats]);
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-card text-card-foreground rounded-xl border border-border p-5 flex flex-col justify-between shadow-xs">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-border">
              <div>
                <h3 className="text-sm font-bold text-foreground">EC location map by approval status</h3>
                <p className="text-[11px] text-muted-foreground mt-0.5">Dot size = recorded internal capacity</p>
              </div>
            ...
            <div className="mt-4 flex flex-wrap gap-4.5 pt-3.5 border-t border-border">
              <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider w-full">Approval Status</span>
            ...
            <div className="space-y-3.5">
              {provinceBreakdown.map((prov) => {
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between text-xs font-semibold text-foreground mb-1">
                      <span className={`group-hover:text-primary transition-colors flex items-center gap-1.5 ${isSelected ? "text-primary font-bold" : ""}`}>
                        {prov.name}
