...
import { X, Tent, Users, Shield, CheckCircle2 } from "lucide-react";
...
export default function LeafletMapView({
  ...
  useEffect(() => {
    ...
    locations.forEach((loc) => {
      ...
      const cap =
        (loc as any).capacity_hhs || (loc as any).capacity_persons || (loc as any).hhs || 20;
      ...
      marker.on("click", () => {
        if (onCoordinatesChange) {
          onCoordinatesChange({ latitude: loc.latitude, longitude: loc.longitude });
        }
      });
        ...
        <div className="bg-card text-card-foreground rounded-xl border border-border p-5 shadow-xs space-y-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Click province to filter map view
            </p>
          </div>
            ...
            {provinceBreakdown.map((prov) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    if (onProvinceChange) {
                      onProvinceChange(isSelected ? null : prov.name);
                    }
                    if (onCoordinatesChange) onCoordinatesChange(null);
                  }}
