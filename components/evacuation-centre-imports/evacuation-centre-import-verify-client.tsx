...
export default function EvacuationCentreImportVerifyClient({ id }: Props) {
  ...
  const {
    ...
    queryKey: ["evacuation-centre-import-detail", id, token],
    queryFn: () => evacuationCentreImportService.get(id, token),
  ...
  const verifyMutation = useMutation({
    mutationFn: async () => {
      ...
      return evacuationCentreImportService.verify(id, selectedStatus, commentText, token);
    },
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["evacuation-centre-import-detail", id],
      });
      ...
      <div className="max-w-md w-full mx-auto px-4 py-16 animate-fadeIn space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Please log in with an authorized account to verify and review evacuation centre import
            files.
          </p>
        </div>
        <LoginCard />
  ...
  return (
    <div className="max-w-3xl w-full mx-auto px-4 sm:px-6 py-12 animate-fadeIn space-y-6">
      ...
      <div className="flex items-center justify-between">
        <Link
          href="/assement/evacuation-centre-assessment-form/imports"
        ...
        >
          <ChevronLeft className="w-3.5 h-3.5" /> Back to Evacuation Centre Imports
        </Link>
