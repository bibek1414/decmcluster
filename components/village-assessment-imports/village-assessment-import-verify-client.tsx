...
export default function VillageAssessmentImportVerifyClient({ id }: Props) {
  ...
  const {
    ...
    queryKey: ["village-assessment-import-detail", id, token],
    queryFn: () => villageAssessmentImportService.get(id, token),
  ...
  const verifyMutation = useMutation({
    mutationFn: async () => {
      ...
      return villageAssessmentImportService.verify(id, selectedStatus, commentText, token);
    },
    onSuccess: () => {
      toast.success(`Import successfully ${selectedStatus}!`);
      setCommentText("");
      setSelectedStatus(null);
      queryClient.invalidateQueries({
        queryKey: ["village-assessment-import-detail", id],
      });
      ...
      <div className="max-w-md w-full mx-auto px-4 py-16 animate-fadeIn space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Please log in with an authorized account to verify and review village assessment import
            files.
          </p>
        </div>
        <LoginCard />
  ...
  // State 3: User is logged in but has no permission (e.g. not Superadmin)
  if (!canVerify) {
    return (
        ...
        <div className="bg-card border border-border rounded-2xl p-6 text-center space-y-6 shadow-sm">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Your account <strong>{user?.email}</strong> does not have permission to verify village
              assessment imports. Please request verification rights from the administrator.
            </p>
          </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-8 text-center space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            {(error as Error)?.message ||
              "The requested import entry could not be retrieved. It may have been deleted."}
          </p>
  ...
  return (
    <div className="max-w-3xl w-full mx-auto px-4 sm:px-6 py-12 animate-fadeIn space-y-6">
      ...
      <div className="flex items-center justify-between">
        <Link
          href="/assement/village-assessment/imports"
        ...
        >
          <ChevronLeft className="w-3.5 h-3.5" /> Back to Village Assessment Imports
        </Link>
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold text-foreground truncate max-w-sm sm:max-w-md">
                {item.file ? item.file.split("/").pop() : "import_file"}
              </p>
              <p className="text-[10px] text-muted-foreground font-semibold">Excel/CSV Dataset</p>
            </div>
          </div>
          <Button
            variant="outline"
          ...
          <form onSubmit={handleVerifySubmit} className="pt-2 space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                placeholder={
                  selectedStatus === "returned"
                    ? "Specify the reasons for returning this dataset (required)..."
