...
export default function ReportVerifyClient({ id }: Props) {
  ...
  const {
    ...
    queryKey: ["report-detail", id, token],
    queryFn: () => reportService.get(id, token),
  ...
  const verifyMutation = useMutation({
    mutationFn: async () => {
      ...
      return reportService.verify(id, selectedStatus, commentText, token);
    },
    onSuccess: (updated) => {
      toast.success(`Report successfully ${selectedStatus}!`);
      setCommentText("");
      ...
      <div className="max-w-md w-full mx-auto px-4 py-16 animate-fadeIn space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Please log in with an authorized account to verify and review situational reports.
          </p>
        </div>
        <LoginCard />
      </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-6 text-center space-y-6 shadow-sm">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              ...
              situational reports. Please request verification rights from the administrator.
            </p>
          </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-8 text-center space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            {(error as Error)?.message ||
              "The requested report could not be retrieved. It may have been deleted."}
          </p>
  ...
  return (
    <div className="max-w-3xl w-full mx-auto px-4 sm:px-6 py-12 animate-fadeIn space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          <ChevronLeft className="w-3.5 h-3.5" /> Back to Situational Reports
        </Link>
