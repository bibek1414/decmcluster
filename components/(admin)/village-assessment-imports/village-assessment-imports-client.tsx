...
import {
  useVillageAssessmentImports,
  useUploadVillageAssessmentImport,
  useDeleteVillageAssessmentImport,
  useReverifyVillageAssessmentImport,
  useUploadNewVillageAssessmentImport,
} from "@/hooks/use-village-assessment-imports";
...
export default function VillageAssessmentImportsClient() {
  ...
  // Fetch village assessment imports list
  const { data, isLoading, isPlaceholderData, error } = useVillageAssessmentImports(
    page,
    token,
    debouncedSearch,
  );
  ...
  // Mutations
  const uploadMutation = useUploadVillageAssessmentImport();
  const deleteMutation = useDeleteVillageAssessmentImport();
  const reverifyMutation = useReverifyVillageAssessmentImport();
  const uploadNewMutation = useUploadNewVillageAssessmentImport();
  ...
  const handleUploadSubmit = (e: React.FormEvent) => {
    ...
    uploadMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("Village assessment import file uploaded successfully!");
          setIsUploadOpen(false);
          setUploadName("");
  ...
  return (
    <div className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fadeIn relative">
      <div className="bg-transparent sm:bg-card text-card-foreground sm:rounded-2xl p-0 sm:p-6 md:p-8 border-0 sm:border border-border space-y-6">
        ...
        >
          <ChevronLeft className="h-4 w-4" /> Back to Assessment Dashboard
        </Link>

        <PageHeader
          title="Village Assessment Imports"
          description={
            <div className="flex flex-col gap-0.5">
              <span>Upload, manage, and verify village assessment spreadsheets</span>
            </div>
          }
          actions={
            canAdd && (
        ...
        <div className="relative min-h-[200px]">
          {isLoading || isPlaceholderData ? (
            ...
            <EmptyState
              ...
              title="No village assessment imports found"
              description={
                canAdd
                  ? "Upload your first village assessment Excel/CSV list to submit for verification."
                  : "No village assessment list imports have been uploaded yet."
              }
              action={
                canAdd ? (
                  <Button
            ...
            <form onSubmit={handleUploadSubmit} className="space-y-4">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  placeholder="e.g. Shefa Village Assessments July 2026"
