...
export default function DisplacementImportVerifyClient({ id }: Props) {
  ...
  const {
    ...
    queryKey: ["displacement-import-detail", id, token],
    queryFn: () => displacementImportService.get(id, token),
  ...
  const verifyMutation = useMutation({
    mutationFn: async () => {
      ...
      return displacementImportService.verify(id, selectedStatus, commentText, token);
    },
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["displacement-import-detail", id],
      });
  ...
  return (
    <div className="max-w-3xl w-full mx-auto px-4 sm:px-6 py-12 animate-fadeIn space-y-6">
      ...
      <div className="flex items-center justify-between">
        <Link
          href="/assement/displacement-data/imports"
        ...
        >
          <ChevronLeft className="w-3.5 h-3.5" /> Back to Displacement Imports
        </Link>
